...
describe "#{Chef::Provider::PushitRails}.create" do
  ...
  context 'when we add env variables from the lwrp' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new(:step_into => %w(pushit_rails pushit_app pushit_base))
    end

    let(:override_instance) do
      Chef::Pushit::App.new 'rails-example', override_hash
    end

    let(:union_instance) do
      Chef::Pushit::App.new 'rails-example', union_hash
    end
    ...
    before do
      allow(override_instance).to receive(:version).and_return(app_version)
      allow(union_instance).to receive(:version).and_return(app_version)

      allow(Chef::Pushit::App).to receive(:new).with('rails-example', override_hash).and_return(
        override_instance
      )

      allow(Chef::Pushit::App).to receive(:new).with('rails-example', union_hash).and_return(
        union_instance
      )

      allow(Chef::Pushit::App).to receive(:new).with('rails-example', {}).and_return(
        union_instance
      )
    end

    it 'gives the lwrp precedence' do
      chef_run.node.set[:pushit_test]['rails-example'][:config] = override_hash
    ...
    it 'does a union of the data bag and lwrp' do
      chef_run.node.set[:pushit_test]['rails-example'][:config] = union_hash
      chef_run.converge('pushit_test::rails')

      expect(chef_run).to render_file('/opt/pushit/apps/rails-example/shared/env').with_content(/EXTRA/)
      expect(chef_run).to render_file('/opt/pushit/apps/rails-example/shared/env').with_content(/FOO/)
    end
  end
end
